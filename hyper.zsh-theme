...
PROMPT='%{$fg_bold[white]%}[%{$reset_color%}%{$fg_bold[blue]%}%n-%m%{$reset_color%}%{$fg_bold[white]%}]%{$reset_color%} %{$fg_bold[cyan]%}%c $(git_prompt_info)%{$reset_color%}$(_git_time_since_commit)$(git_prompt_status)${_return_status}%{%G$%} '
local _return_status="%{$fg_bold[red]%}%(?..%{%G⍉%} )%{$reset_color%}"
